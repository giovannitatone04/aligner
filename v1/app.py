...
base = Path("/mnt/data/aligner_streamlit_app")
pages = base / "pages"
...
with st.sidebar:
    st.title("🦷 Aligner Predictor")
    st.caption("Livello 1 evidence-based + Livello 2 esplorativo")
    st.markdown("---")
    st.subheader("Input")
...
st.caption("Strumento dimostrativo per la stima della suscettibilità estetica dei principali materiali per aligner.")

tab1, tab2, tab3 = st.tabs(["🏠 Predizione", "ℹ️ Come funziona", "📘 Note metodologiche"])
...
with tab1:
    if run_button:
        try:
            ...
            c1.metric("Materiale", level1.get("material_brand"))
            c2.metric("Polimero", level1.get("polymer"))
            c3.metric("ΔE totale", level1.get("total_estimated_deltaE_numeric_agents_only"))
            c4.metric("Global risk", level2["global_summary"].get("global_risk_score"))
...
with tab2:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader("Cosa fa questa app")
    st.write(
        "Questa webapp integra due livelli. Il livello 1 usa dati strutturati dalla letteratura "
        "peer-reviewed per associare materiale, polimero, agente pigmentante e proprietà del materiale. "
        "Il livello 2 trasforma questi risultati in una sintesi numerica più leggibile."
    )
    st.markdown("### Flusso logico")
    st.write("1. L’utente inserisce materiale e abitudini di esposizione.")
    st.write("2. Il livello 1 stima il ΔE usando i dati disponibili nel database.")
    st.write("3. Se mancano dati diretti del brand, può usare un proxy per famiglia polimerica.")
    st.write("4. Il livello 2 costruisce score esplorativi di staining, meccanica, superficie e rischio globale.")
    st.markdown(
        '<p class="small-note">Il focus principale del modello resta estetico. Gli score di livello 2 sono una modellizzazione trasparente, non un algoritmo validato clinicamente.</p>',
        unsafe_allow_html=True
    )
    st.markdown("</div>", unsafe_allow_html=True)
...
with tab3:
    st.markdown('<div class="section-card">', unsafe_allow_html=True)
    st.subheader("Avvertenze metodologiche")
    st.write("• Il ΔE numerico deriva solo dagli agenti con dati quantitativi disponibili nel database.")
    st.write("• Il fumo è trattato soprattutto come fattore superficiale separato.")
    st.write("• I tempi standardizzati per singolo consumo sono parametri del modello, non costanti peer-reviewed per aligner.")
    st.write("• Gli score di livello 2 servono a sintetizzare e confrontare, non a sostituire la misura sperimentale.")
    st.markdown("</div>", unsafe_allow_html=True)
"""
...
st.set_page_config(page_title="Fonti", page_icon="📚", layout="wide")

st.title("Fonti e dataset usati")
st.caption("Pagina secondaria con le fonti strutturate presenti nei CSV del progetto.")
...
with tab1:
    st.subheader("materials_master_v2.csv")
...
with tab2:
    st.subheader("staining_evidence_v2.csv")
...
with tab3:
    st.subheader("mechanical_evidence_v2.csv")
...
with tab4:
    st.subheader("thermoforming_fit_surface_v2.csv")
...
st.markdown("---")
st.markdown(
    "Questa pagina legge direttamente i CSV del progetto. "
    "Per coerenza scientifica, la webapp principale mostra la predizione, "
    "mentre qui puoi verificare la base dati e le fonti associate."
)
"""

(base / "app.py").write_text(app_code, encoding="utf-8")
(pages / "1_Fonti.py").write_text(sources_code, encoding="utf-8")

print("Created:")
print(base / "app.py")
print(pages / "1_Fonti.py")
